...
- [x] 文档列表页：进入某库后平铺列出库内文档，含新建/编辑/删除入口
      ...
      文档库列表每行增加「管理文档」入口跳转
- [x] 文档树管理页：树形展示、拖拽排序、层级调整（`DocList.vue` 用 `@he-tree/vue` 的
      `Draggable` 渲染，`buildDocTree`（`utils/doc-tree.ts`，含单测）按 `spec.parent`/`priority`
      组树；拖拽后 `flattenPositions` diff 出受影响节点，只对 parent/priority 变化者走
      `DocEndpoint` PUT 即时保存，`Promise.all` 并发 + invalidate 重拉校正；节点行内
      新建子文档/编辑/删除。树需全量故去分页，上限 200 并 log+Toast 提示）
