...
- [x] 文档渲染设置：浅色 / 深色模式可分别配置自定义 CSS 地址、正文容器 class 与
      highlight.js 代码主题；未配置内容 CSS 时使用基础回退样式。其余设置包含代码行号、自动空格、自动链接、脚注、Mark、
      术语修正、段首缩进、图表代码块渲染与 RaTeX 数学公式渲染（透明 Canvas、颜色跟随正文，失败时回退 Vditor）。当前这些设置**仅作用于前台文档页面**，
...
- [x] README 与项目计划持续同步当前能力（已补充明暗独立自定义 Markdown CSS、
      渲染开关及主题跟随规则）
...
| 渲染配置          | 当前为插件级全局配置，自定义内容 CSS、正文 class 与代码主题按明暗模式区分；内容 CSS 留空时使用基础回退样式，其余开关共享且只作用于前台文档页面 |
