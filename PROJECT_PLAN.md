...
- [x] 前台主题跟随 Halo 当前主题设置：文档页不再跟随浏览器/系统主题，而是与站点主题状态保持一致
...
- [x] 文档渲染设置：已把文档页面渲染相关配置纳入插件设置页统一管理，包括内容主题、代码主题、
      代码行号、自动空格、自动链接、脚注、Mark、术语修正、段首空两格、代码块即时渲染、
      公式块即时渲染等。当前这些设置**仅作用于前台文档页面**，不影响后台编辑器行为
...
- [x] README 与项目计划持续同步当前能力（已补充文档设置导出 / 加载与文档内容恢复说明）
...
| 渲染配置          | 当前为插件级全局配置，且只作用于前台文档页面；后续再评估文档库级覆盖         |
