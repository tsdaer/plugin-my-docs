...
- 插件设置与仪表盘：支持默认排序、默认文档库、文档库首页布局、明暗模式独立的文档渲染主题，以及设置导出 / 加载与文档内容恢复，并提供后台统计卡片
- 文档阅读增强：支持同库文档标题短链接跳转、详情页正文大纲、滚动高亮、移动端折叠目录，以及跟随 Halo / 系统明暗模式切换内容与代码主题
...
文档设置页可单独配置浅色与深色模式下的 Markdown 内容主题和代码高亮主题。内置内容主题包括
Light、Dark、WeChat 与 Ant Design，也可以为两种模式分别填写第三方 CSS 地址和正文容器 class，
例如使用 `github-markdown-css` 时将容器 class 设置为 `markdown-body`。

前台阅读页优先读取 Halo 主题写入页面或本地存储的明暗状态；当 Halo 主题选择 `auto` 或没有提供明确状态时，
跟随浏览器的 `prefers-color-scheme`。主题状态变化后，内容主题与代码主题会在不重新生成正文 HTML 的情况下切换。
...
- 文档设置页支持为浅色 / 深色模式分别配置内置或自定义 Markdown 内容主题及代码高亮主题；自定义主题通过 HTTPS / 站内 CSS 地址与正文容器 class 接入
...
- 文档内容主题和代码主题按浅色 / 深色模式独立配置；优先跟随 Halo 当前主题，`auto` 或无明确信号时跟随浏览器系统主题
