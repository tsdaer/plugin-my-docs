...
- **媒体代理大小上限**：单个文件的上限，超过即中断。

代理是**流式转发**：上游响应头一到就回给浏览器，视频等大文件边下边播，首帧不用等
完整下载；Range 请求（拖动进度条）原样透传到上游，只取所需片段；播放器放弃旧请求时，
对上游的下载也随之取消，服务端不落任何中间副本。若地址本身已带 `X-Amz-Signature`
一类预签名参数，代理不会再叠加签名凭证，避免双重鉴权被 S3 拒绝。
...
并把响应类型改成推断结果再回给浏览器；明确危险的类型（`text/html`、`image/svg+xml` 等）
即使扩展名像媒体也一律拒绝，其余响应统一带 `X-Content-Type-Options: nosniff`。
